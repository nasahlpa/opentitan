...
"""Generates cryptolib_info.c for a cryptolib build."""
...
def generate_cryptolib_info_c_source(scm_revision: int) -> str:
    """Return the contents of a C source file that defines `kCryptoLibInfo`.
...
#include "sw/device/lib/crypto/include/cryptolib_info.h"

#include "sw/device/lib/base/macros.h"

const cryptolib_info_t kCryptoLibInfo = {{
  .scm_revision = (cryptolib_info_scm_revision_t){{
    .scm_revision_low = {scm_revision_low:#010x},
    .scm_revision_high = {scm_revision_high:#010x},
  }},
  .version = (uint32_t)kCryptoLibInfoVersion1,
}};
"""
...
def write_source_file(outdir: Path, contents: str) -> Path:
    """Creates cryptolib_info.c in `outdir`. Returns the path to the new file."""

    source_out_path = outdir / "cryptolib_info.c"
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="cryptolib_info")
    ...
    generated_source = generate_cryptolib_info_c_source(version)
